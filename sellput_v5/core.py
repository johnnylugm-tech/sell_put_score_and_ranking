...
class ScoreResult:
    ...
    warnings: List[str] = None  # P0修正③：近52W低點等特殊警告
    option: OptionData
    ...
    def fetch_option_data(self, stock: StockData) -> OptionData:
        ...
        for attempt in range(3):
            try:
                ...
                iv = atm_put['impliedVolatility'] * 100 if not pd.isna(atm_put['impliedVolatility']) else 0
                bid = atm_put['bid']
                ask = atm_put['ask']
                oi = atm_put['openInterest']
                spread = (ask - bid) / ((ask + bid) / 2) * 100 if (ask + bid) > 0 else 0

                # Black-Scholes Delta
                try:
                    S, K, T, r, sigma = price, strike, selected_dte/365, 0.045, iv/100
